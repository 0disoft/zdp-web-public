...
`glossary:generate`는 `../../contracts/zdp-libs-ts/glossary/terms/*.yaml` 공통 용어와 `glossary/terms/*.yaml` 사이트 전용 용어를 `zdp-platform-devex`의 glossary manifest 빌더로 함께 읽고 `src/content/glossary-manifest.json`을 만든다. Astro 런타임은 이 JSON만 소비하며, `src/content/glossary.ts`에는 용어 본문을 중복해서 넣지 않는다.
...
용어 설명은 hover tooltip이 아니라 click sheet 패턴으로 유지한다. 데스크톱에서는 오른쪽 sheet, 모바일에서는 bottom sheet로 열리고, sheet root는 stable `term_id`와 `data-zdp-ad-exclude`를 남긴다. Sheet 안에는 광고 slot을 넣지 않고, 광고 실험은 별도 용어 detail page 계약에서만 다룬다. Vault, 감사 로그, OKLCH처럼 여러 공개 사이트가 반복해서 쓰는 용어는 `zdp-libs-ts/glossary/terms/public.yaml`에 추가하고, 이 사이트에서만 쓰는 용어만 `glossary/terms/public.yaml`에 둔다. 공통 용어의 관련 화면 경로는 이 저장소의 manifest 빌더에서 term id별로 붙인 뒤 `bun run glossary:generate`로 런타임 manifest를 갱신한다.
